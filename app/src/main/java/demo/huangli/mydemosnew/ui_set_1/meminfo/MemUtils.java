...
public class MemUtils {
    ...
    public static double getFreeMemPercentage() {
        long[] memInfo = getMemInfo();
        if (memInfo != null && memInfo.length >= 4) {
            long freeMem = memInfo[1] + memInfo[2] + memInfo[3];
            long totalMem = memInfo[0];
            return (1.0 * freeMem) / totalMem;
        }
        return 0;
    }

    public static double getNativePercentage(){
        long[] memInfo = getMemInfo();
        if (memInfo != null && memInfo.length >= 4) {
            long totalMem = memInfo[0]*1024*1024;
            long freeMem= Runtime.getRuntime().totalMemory();
            return (totalMem-freeMem)*0.6 / (totalMem);
        }
        return 0;
    }
